+++
path = "2015/02/28/code_coverage_in_the_monte_tests"
title = "Code Coverage in the Monte Tests"
date = 2015-02-28
...
author = "E. Dunham"
+++
My schoolwork is particularly bad this weekend, so I'm procrastinating by learning to analyze the test coverage of a moderately complex Python codebase. Specifically, the [reference implementation](https://github.com/monte-language/monte) of the [Monte programming language](http://monte.readthedocs.org/en/latest/). This effort is hampered only slightly by the fact that I've never done much with code coverage tools before.


# Nedbat's tool

Some cursory searching and reading the resulting blogs points out that the Ned Batchelder's [coverage](http://nedbatchelder.com/code/coverage/) is the most-recommended Python code coverage analyser. You run it on your program; it spits out a report of any lines that weren't executed and optionally makes that report into pretty HTML.
...
The closest thing I could find to an explanation of the `.cover` files was a [twisted man page](https://developer.apple.com/library/mac/documentation/Darwin/Reference/ManPages/man1/trial.1.html) in the Mac OSX 10.9 docs. I don't have a man page for Trial on my own system, perhaps because it's installed in a virtualenv rather than through my OS's package manager.
...
After reading the Twisted developers [arguing about including coverage.py](https://twistedmatrix.com/trac/ticket/4374) and explaining its use [on the mailing list](http://twistedmatrix.com/pipermail/twisted-python/2012-April/025487.html), I realized that Nedbat's [coverage](http://nedbatchelder.com/code/coverage/) is probably the "right answer" after all.
...
Note that the `-i` flag is necessary because of [an issue](http://stackoverflow.com/questions/2386975/no-source-for-code-message-in-coverage-py) where `coverage` thinks it should have source for files that it actually shouldn't, in my case:
