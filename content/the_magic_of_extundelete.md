+++
path = "2015/03/02/the_magic_of_extundelete"
title = "The Magic of Extundelete"
date = 2015-03-02
...
To spoil the end of this saga, I found that it was easier to recover a deleted file than an overwritten one. Although the Ubuntu Forums have [instructions](http://ubuntuforums.org/showthread.php?t=2113182) for recovering an overwritten file, they did not work for me. I suspect it may be because the file was overwritten with the output of `tar`, rather than being overwritten with an empty file.
...
The [instructions](http://ubuntuforums.org/showthread.php?t=2113182) I was following said to mount the partition containing the overwritten file, so I mounted the `sda` partitions one at a time until finding which one contained Linux (`/dev/sda4`). As per the instructions, I removed the overwritten file and then unmounted the partition.
...
Despite following the [instructions](http://ubuntuforums.org/showthread.php?t=2113182) as correctly as I could at 1am, the recovered `code_generator.c` file was still overwritten with the gibberish of the errant `tar` command.
...
>
> -- (from the [Vim docs](http://vimdoc.sourceforge.net/htmldoc/recover.html))
