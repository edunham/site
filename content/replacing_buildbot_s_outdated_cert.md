+++
path = "2015/07/17/replacing_buildbot_s_outdated_cert"
title = "Outage postmortem: Replacing Rust Buildbot's outdated cert"
date = 2015-07-17
...
A quick check fo the [stunnel docs](https://www.stunnel.org/howto.html) shows that `verify=3` is the strictest setting, and will fail if the locally installed cert isn't right. This supports the hypothesis that our cert might be expired. On the buildmaster, I found the cert and examined its metadata:
...
The [stunnel docs](https://www.stunnel.org/howto.html) give most of the relevant incantation. Since no file in our buildbot directory is named precisely `stunnel.conf`, `make cert` doesn't quite work right. But it works fine to manually run a variant of the command given in the docs:
...
Finally, I [updated the repository](https://github.com/rust-lang/rust-buildbot/pull/21) with the new certificate.
...
![AWS EC2 console showing AMI search and launch button](/pictures/aws-ami-launch.png)
...
Getting into the Windows hosts requires [using RDP](http://docs.aws.amazon.com/AWSEC2/latest/WindowsGuide/connecting_to_windows_instance.html) after setting up an SSH tunnel to the bastion. Since I was on airport wifi at the time, I had a teammate stick the cert onto the Windows instances instead.
