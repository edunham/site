+++
path = "2015/03/05/hieroglyph2beamer_with_pandoc"
title = "hieroglyph2beamer with Pandoc"
date = 2015-03-05
...
author = "E. Dunham"
+++
I've played with getting Sphinx to generate PDFs [before](http://edunham.net/2015/02/24/making_a_pdf_of_hieroglyph_slides.html), and while `rst2pdf` generates a PDF with all the notes and pictures present, the results aren't as beautifully typeset as I've come to expect from LaTeX.

This made me wonder whether any tool exists to convert Hieroglyph slides into [Beamer](http://texdoc.net/texmf-dist/doc/latex/beamer/doc/beameruserguide.pdf) presentations.


Note

Skip to the [end](#end) if you just want the code for converting Hieroglyph sources to Beamer slides.

# Why not just write LaTeX?

I sometimes do write LaTeX directly, such as for my [resume](https://github.com/edunham/resume). LaTeX is the right choice when a project requires:
...
It generates pretty Beamer slides, just like that! If you want to get fancier with it, [this blog post](http://andrewgoldstone.com/blog/2014/12/24/slides/) is an interesting read.
...
Beamer has an `allowframebreaks` option, which makes sure all your content is visible by wrapping it to as many slides as needed. I learned how to inject a custom preamble from [agoldst's lecture notes repo](https://github.com/agoldst/tex/tree/master/lecture-slides/notes).
...
Sphinx and Hieroglyph allow the use of pipes (`|`) to force a blank line in html output, which I often use to align my images aesthetically in my slides. However, this use of pipes is neither mentioned in the [rst spec](http://docutils.sourceforge.net/docs/ref/rst/restructuredtext.html) nor supported by Pandoc.

Although the functionality of ignoring pipes could probably be implemented as a [filter](http://johnmacfarlane.net/pandoc/scripting.html), I decided to take the path of less Haskell-writing and leverage Pandoc's ability to behave like a nice Unix utility.:
...
I'm using the preamble trick mentioned above to customize each figure. I found the parameters for scaling only those images which would otherwise be too large on [stackoverflow](http://tex.stackexchange.com/questions/11954/automatically-scale-big-and-small-graphics-for-beamer-presentations). Renewing the `\includegraphics` command is [tricky](http://tex.stackexchange.com/questions/79724/resize-all-images-in-latex-to-a-percentage-width), because it has an optional parameter, but it can be done with the `letltxmacro` package. I added these lines to my `preamble.tex`:
