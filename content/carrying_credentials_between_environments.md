+++
path = "2015/09/29/carrying_credentials_between_environments"
title = "Carrying credentials between environments"
date = 2015-09-29
...
See [here](http://www.unixwiz.net/techtips/ssh-agent-forwarding.html) for pretty pictures explaining in more detail how agent forwarding works.
...
Did you see what just happened there? An arbitrary user with sudo on `B` just gained access to all the keys added to ssh-agent on `A`. [Simon](http://exyr.org/about/) pointed out that the right way address this issue is to use [ProxyCommand](https://heipei.github.io/2015/02/26/SSH-Agent-Forwarding-considered-harmful/) instead of agent forwarding.
