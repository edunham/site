+++
path = "2015/06/26/how_to_find_a_buildbot_slave_s_ip"
title = "How to find a Buildbot slave's IP"
date = 2015-06-26
...
I know that the slave whose IP I want is running builds successfully, which means it has an [stunnel](https://www.stunnel.org/index.html) back to the master.
...
It's also possible, with a bit more typing, to perform [reverse DNS lookups](http://www.dnsstuff.com/reverse-dns-faq/) with `dig`. You just reverse the order of the blocks of digits in the IP address, and add `.in-addr.arpa` to the end. For instance, `abc.def.ghi.jkl` would become `jkl.ghi.def.abc.in-addr.arpa`.
