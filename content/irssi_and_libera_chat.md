+++
path = "2021/05/25/irssi_and_libera_chat"
title = "irssi and libera.chat"
date = 2021-05-25
...
The fix is to pop open an IRC client locally (or use web IRC), connect to Libera without SASL, and register one's nick and password. After verifying one's email address over the regular connection, the network can be reached via SASL from anywhere using the registered nick as the <span class="title-ref">username</span> and the nickserv password as the <span class="title-ref">password</span>.
