+++
path = "2015/11/17/installing_rust_without_root"
title = "Installing Rust without root"
date = 2015-11-17
...
Pick one of the two following sets of directions. I'd recommend using [Multirust](https://github.com/brson/multirust), because it automatically checks the packages it downloads and lets you switch between Rust versions trivially.


# Without multirust

If you just want one version of Rust, [this blog post](https://www.codejam.info/2015/03/portable-rust-installation.html) by Valérian Galliat has a fix in 7 lines:
...
To [verify the GPG signature](https://www.gnupg.org/gph/en/manual/x135.html), you'll also need a copy of the Rust project's public key. This key is available through several channels:

-   [on the Rust website](https://www.rust-lang.org/rust-key.gpg.ascii), available only over HTTPS.
-   [on keybase.io](https://keybase.io/rust), correlated to Rust's Twitter account and URL. Don't worry, we authenticated the key by signing a string from Keybase with it locally. We don't trust them to ever see our private key.
-   [on GitHub](https://github.com/rust-lang/rust-www/blob/master/rust-key.gpg.ascii), in the website's repository.
...
It means you don't have permissions to write to `/etc/ld.so.cache`. Until [this issue](https://github.com/brson/multirust/issues/113) gets fixed, the easiest workaround to lacking those permissions is to change the script called by the installer to pass `-C` to `ldconfig`:
