+++
path = "2015/02/24/making_a_pdf_of_hieroglyph_slides"
title = "Making a PDF of Hieroglyph slides"
date = 2015-02-24
...
According to the [Sphinx tutorial](http://sphinx-doc.org/tutorial.html), the recommended way to build PDFs of Sphinx documentation is through the [LaTeX builder](http://sphinx-doc.org/builders.html#sphinx.builders.latex.LaTeXBuilder). According to the docs, this should enable me to simply use `make latexpdf`.

I had installed `texlive-core` to do basic PDF rendering, but it did not include certain required packages such as `titlesec.sty`. At the [Arch Wiki's suggestion](https://wiki.archlinux.org/index.php/TeX_Live), I installed `texlive-most`. The fonts took a long time to download, but it will prevent further problems with missing packages.
...
A thread on [StackExchange](http://tex.stackexchange.com/questions/83440/inputenc-error-unicode-char-u8-not-set-up-for-use-with-latex) theorizes that the `\u8` represents a "no-break space" character, so I think it's safe to strip it out entirely. A rather heavy-handed fix [from elsewhere](http://stackoverflow.com/questions/8562354/remove-unicode-characters-from-textfiles-sed-other-bash-shell-methods) is to forcibly convert the whole file from UTF-8 to ASCII:
...
According to the [Sphinx docs](http://sphinx-doc.org/builders.html), there exists a builder which converts rst directly into pdf.
...
This means that `rst2pdf` wants some extra configuration information. [The manual](http://ralsina.me/static/manual.pdf) doesn't discuss Sphinx-specific configuration, but [a Sphinx issue](https://bitbucket.org/birkenfeld/sphinx/issue/999/create-pdf-using-rst2pdf) explains that you need one more line in `conf.py`:
